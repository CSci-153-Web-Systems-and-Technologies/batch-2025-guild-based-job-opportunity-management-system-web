    ...
    ;(async () => {
      try {
        ...
        try {
          ...
          if (!text) return
          const json: ProgressResponse = JSON.parse(text)
          setPercent(json.progress?.percent ?? 0)
          setTotalExp(json.stats?.xp ?? 0)
        } catch (parseErr) {
          console.error('[LevelProgressBar] Failed to parse stats response:', parseErr)
        }
      } catch (err) {
        console.error('[LevelProgressBar] Fetch error:', err)
      }
    })()
